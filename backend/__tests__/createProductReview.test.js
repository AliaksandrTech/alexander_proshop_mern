 ...
 * (backend/controllers/productController.js, line ~113).
 *
 * These tests pin down the controller's CURRENT behavior — including bugs.
 * They are not specifications of "what should happen"; they are a safety net
 * so refactors don't change behavior silently. Tests that capture buggy
 * behavior are marked with a "BUGGY BEHAVIOR" comment.
 *
 * Run: npx jest backend/__tests__/createProductReview.test.js
 * (requires `npm i -D jest` and `"type": "module"` already set in package.json)
 */
